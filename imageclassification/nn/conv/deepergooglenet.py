...
class DeeperGoogLeNet:
    @staticmethod
    def conv_module(x, K, kY, kY, stride, chanDim, padding="same", reg=0.0005, name=None):
        # Initialize the names for each layer [CONV -> BN -> RELU]
        (convName, bnName, actName) = (None, None, None)

        # If one sent the name as parameters, prepend it
        if name is not None:
            convName = name + "_conv"
            bnName = name + "_bn"
            actName = name + "_act"
        
        # Define the pattern [CONV => BN => RELU]
        x = Conv2D(
            k, (kX, kY), strides=strides, padding=padding, kernel_regularizer=l2(reg),
            name=convName 
        )(X)
    ...
    def inception_module(x, num1x1, num3x3Reduce, num3x3, num5x5Reduce, num5x5, num1x1Proj, 
        ...
        # Define first Model Branch
        first = DeeperGoogLeNet.conv_module(
            x, num1x1, 1, 1, (1, 1), chanDim, reg=reg, 
            name=stage+"_first"
        )
